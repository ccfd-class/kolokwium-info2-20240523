...
double F0(double y) {
  const double e1 = lagrange(xpom, E1, n, x_);
  const double e2 = lagrange(xpom, E2, n, x_);
  return exp(-e1 * y * y) - e2 * y * y;
}

double y(double x) {
  x_ = x;
  int _;
  return bisec(0., 100., F0, 1e-6, &_);
}

double rozwiazanie() { return simpson(x1, x2, y, 100) / (x2 - x1); }
